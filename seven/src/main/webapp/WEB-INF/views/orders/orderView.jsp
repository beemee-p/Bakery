...
<div class="container orderinfo">
	<div class="orderinfo_title"><h4>주문정보 상세</h4></div>
		...
		<c:if test="${status.first}">
			<table class="orderinfo_table">
				...
				<tr>
					...
					<td>${list.sState}
						<c:if test="${list.sState eq '배송전'}">&nbsp;&nbsp;
							<button type="button" class="button" onclick="location.href='deliver_ing.do?oId=${list.oId}'">
								배송중
							</button>&nbsp;&nbsp;
							<button type="button" class="button" onclick="location.href='deliver_cancelAdmin.do?oId=${list.oId}'">
								구매취소
							</button>
						</c:if>
						<c:if test="${list.sState eq '배송중'}">
							<button type="button" class="button" onclick="location.href='deliver_fini.do?oId=${list.oId}'">
								배송완료
							</button>
						</c:if>
					</td>						
...
<!-- ################### 주문상품 정보 #################### -->
<div class="container">
	<div class="orderinfo_title"><h4>주문상품 상세</h4></div>	
	<c:forEach items="${list}" var="list" varStatus="status">
		<table class="orderinfo_table">
			<tr>
				<th scope="row">상품명</th>
